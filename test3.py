...
driver.get("https://finance.eastmoney.com/a/cgjjj.html")
def index():
    # 设置等待5秒，避免爬虫时被封禁
    driver.implicitly_wait(5)
    # 找到外框的获取里面所有的li标签
    lis = driver.find_elements_by_css_selector(".repeatList ul li")
    for i,li  in enumerate(lis):
            ...
            f = '年'
            time24 = li.find_element_by_css_selector('p.time').text
            d = b + f + time24
            newtext = li.find_element_by_css_selector("p.info").text
            print(d,line,newtext)
            # 获取一个光标
            cursor = conn.cursor()
            # 插入数据,insert into 表（列名，列名，列名）values(值，值，值)这个值主要就是对应的你获取到的值
            sql = 'insert into quantizationsystem_new_int_economic(new_int_time,new_int_sources,new_int_content) values(%s,%s,%s);'
