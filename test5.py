# 'name': '行业研究',
#                     # 同花顺行业研究http://stock.10jqka.com.cn/bkfy_list/
...
driver.get("http://stock.10jqka.com.cn/bkfy_list/")
def index():
    # 设置等待5秒，避免爬虫时被封禁
    ...
    for i,li  in enumerate(lis):
            ...
            # 获取url
            line = li.find_element_by_css_selector(".arc-title a").get_attribute('href')
            b = time.strftime('%Y', time.localtime(time.time()))
            f = '年'
            time24 = li.find_element_by_css_selector('.arc-title span').text
            d = b + f + time24
            newtext = li.find_element_by_css_selector(".arc-title a").text
            print(d,line,newtext)
            # 获取一个光标
            cursor = conn.cursor()
            # 插入数据,insert into 表（列名，列名，列名）values(值，值，值)这个值主要就是对应的你获取到的值
            sql = 'insert into quantizationsystem_new_ind_finace(new_ind_time,new_ind_sources,new_ind_content) values(%s,%s,%s);'
